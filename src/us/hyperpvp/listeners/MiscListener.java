...
public class MiscListener implements Listener {
	...
	public void onNameTag(PlayerReceiveNameTagEvent event) {
		...
		if (HyperPVP.getTeamCycle().containsKey(named)) {
			event.setTag(HyperPVP.getTeamCycle().get(named) + named.getName());
		
		} else {

			if (HyperPVP.getSpectators().contains(named.getName())) {

				event.setTag(ChatColor.AQUA + named.getName());

			}  else if (HyperPVP.getGameSessions().containsKey(named.getName())) {

				Session session = HyperPVP.getSession(named);
				event.setTag(session.getTeam().getColor() + named.getName());
