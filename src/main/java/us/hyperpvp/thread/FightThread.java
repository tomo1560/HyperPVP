...
public class FightThread extends IThread {
	...
	public FightThread() {
		...
		HyperPVP.getMap().getWorld().setTime(0);
		HyperPVP.getMap().getWorld().setStorm(false);
		HyperPVP.getMap().getWorld().setThundering(false);
		HyperPVP.getMap().getWorld().setWeatherDuration(999999);
	...
	public void run() {
		...
		try {
			...
			while (true) {
				...
				if (this.isCancelled() || this.isInterrupted() || HyperPVP.getGameSessions().size() != 0) {
					break;
				} else {
					for (int i = 0; i < 40; i++) {
